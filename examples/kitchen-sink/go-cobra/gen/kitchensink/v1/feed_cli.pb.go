...
func cli_kitchensink_v1_feed_proto_callContext(cmd *cobra.Command) (context.Context, context.CancelFunc) {
...
func cli_kitchensink_v1_feed_proto_usage(err error) error {
	return cli_kitchensink_v1_feed_proto_exitError{2, err}
}

// callErr maps a failed call to an exit code from its context.
func cli_kitchensink_v1_feed_proto_callErr(ctx context.Context, err error) error {
	if err == nil {
		return nil
	}
	switch ctx.Err() {
	case context.DeadlineExceeded:
		return cli_kitchensink_v1_feed_proto_exitError{124, err}
	case context.Canceled:
		return cli_kitchensink_v1_feed_proto_exitError{130, err}
	}
	return cli_kitchensink_v1_feed_proto_exitError{1, err}
}
...
func NewFeedServiceCommand(conn grpc.ClientConnInterface, opts ...FeedServiceOptions) *cobra.Command {
	...
	cmd.SetFlagErrorFunc(func(_ *cobra.Command, err error) error { return cli_kitchensink_v1_feed_proto_usage(err) })
		...
		sub := &cobra.Command{
			...
			RunE: func(cmd *cobra.Command, _ []string) (err error) {
				if example, _ := cmd.Flags().GetBool("example"); example {
					...
					req := &TailRequest{}
					if err := protojson.Unmarshal([]byte("{\"count\":998}"), req); err != nil {
						return err
					}
				...
				ctx, cancel := cli_kitchensink_v1_feed_proto_callContext(cmd)
				defer cancel()
				defer func() { err = cli_kitchensink_v1_feed_proto_callErr(ctx, err) }()
