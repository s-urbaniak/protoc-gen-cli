...
//
// FieldsService echoes each request, so the response shows exactly what
// arrived.
type FieldsServiceClient interface {
	// Scalars carries every proto scalar type.
	...
	// Repeated carries one repeated field per element type.
	Repeated(ctx context.Context, in *RepeatedRequest, opts ...grpc.CallOption) (*RepeatedRequest, error)
	// Maps carries one map field per value type, plus integer and bool keys.
	...
	// Collections carries two message lists.
