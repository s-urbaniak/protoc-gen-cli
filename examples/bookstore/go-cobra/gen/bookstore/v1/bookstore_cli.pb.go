...
func cli_bookstore_v1_bookstore_proto_callContext(cmd *cobra.Command) (context.Context, context.CancelFunc) {
...
func cli_bookstore_v1_bookstore_proto_usage(err error) error {
	return cli_bookstore_v1_bookstore_proto_exitError{2, err}
}

// callErr maps a failed call to an exit code from its context.
func cli_bookstore_v1_bookstore_proto_callErr(ctx context.Context, err error) error {
	if err == nil {
		return nil
	}
	switch ctx.Err() {
	case context.DeadlineExceeded:
		return cli_bookstore_v1_bookstore_proto_exitError{124, err}
	case context.Canceled:
		return cli_bookstore_v1_bookstore_proto_exitError{130, err}
	}
	return cli_bookstore_v1_bookstore_proto_exitError{1, err}
}

// exclusiveFlags returns a usage error when the caller sets more than one flag.
func cli_bookstore_v1_bookstore_proto_exclusiveFlags(fs *pflag.FlagSet, names ...string) error {
	var set []string
	for _, n := range names {
		if fs.Changed(n) {
			set = append(set, "--"+n)
	...
	if len(set) > 1 {
		return cli_bookstore_v1_bookstore_proto_usage(fmt.Errorf("%s are mutually exclusive", strings.Join(set, ", ")))
	}
	return nil
}
	...
	cmd.AddCommand(func() *cobra.Command {
		sub := &cobra.Command{
			...
			RunE: func(cmd *cobra.Command, _ []string) (err error) {
				...
				defer cancel()
				defer func() { err = cli_bookstore_v1_bookstore_proto_callErr(ctx, err) }()
	...
	cmd.AddCommand(func() *cobra.Command {
		...
		sub := &cobra.Command{
			...
			RunE: func(cmd *cobra.Command, _ []string) (err error) {
				...
				if err := cli_bookstore_v1_bookstore_proto_exclusiveFlags(cmd.Flags(), "auction", "author"); err != nil {
					return err
				}
				print, err := outputPrinter(cmd)
				if err != nil {
	...
	cmd.AddCommand(func() *cobra.Command {
		sub := &cobra.Command{
			...
			RunE: func(cmd *cobra.Command, _ []string) (err error) {
				...
				if err := cli_bookstore_v1_bookstore_proto_pumpRequests(ctx, cmd.InOrStdin(), cmd.ErrOrStderr(), send); err != nil && !errors.Is(err, io.EOF) {
					return err
				}
