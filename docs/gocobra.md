...
By default that constructor accepts a `grpc.ClientConnInterface`. Set
`client=connect-go` to generate a constructor that accepts a per-service
`<Service>CLIClient` interface instead. The concrete client generated by
`protoc-gen-connect-go` v1.20.0 satisfies it:
...
```go
client := shelfv1connect.NewShelfServiceClient(httpClient, endpoint, connectOptions...)
root.AddCommand(shelfv1.NewShelfServiceCommand(client))
```

`client` chooses the generated Go client API, not the wire protocol. A
Connect-Go client can be configured to speak Connect, gRPC, or gRPC-Web. The
generated CLI deliberately does not import the generated `...connect` child
package: that package imports the base protobuf package, so importing it back
